...
import {isEmpty, isString, trim} from './json-helpers';
...
export default function error (code, parameters) {
  ...
  if (isEmpty(error)) {
    throw new Error('error: Error code ' + code + ' not found!');
  }
  ...
  if (isEmpty(errorMessage)) {
    throw new Error('error: Error language ' + language + ' not found for code ' + code);
  }
  ...
  return {
    ...
    body: {
      ...
      moreInfo: 'http://www.developer.com.br/errors'
    }
  };
}
